...
import { createPortfolioItem, deletePortfolioItem, updatePortfolioItem } from '@/actions/portfolio'
...
function ImageUploader({
  ...
  const uploadFile = useCallback(async (file: File): Promise<UploadedImage | null> => {
    ...
    try {
      ...
      setUploading((prev) => prev.map((f) => (f.id === id ? { ...f, error: message } : f)))
      return null
    }
  }, [])
  ...
  return (
    <div>
      <label className="block text-xs text-muted mb-1">Фотографии *</label>

      {/* Current images */}
      {images.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-3">
          {images.map((url, i) => (
            <div key={i} className="relative group m-1.5">
              <Image
                src={url}
                alt=""
                width={80}
              ...
              <Button
                ...
                className="absolute -top-1.5 -right-1.5 size-11 sm:size-8 rounded-full bg-destructive text-white text-xs flex items-center justify-center opacity-100 transition-opacity"
              >
                ×
              </Button>
              {i === 0 && (
      ...
      >
        ...
        <p className="text-xs text-muted">
          Перетащите фото или <span className="text-accent font-medium">выберите файлы</span>
        </p>
        <p className="text-[10px] text-(--placeholder)">JPG, PNG, WEBP, HEIC</p>
        <Input
          ref={inputRef}
          type="file"
          accept="image/*"
...
function PortfolioForm({
  ...
  const {
    ...
    defaultValues: {
      ...
      ...(typeof defaultValues?.published === 'boolean'
        ...
        : {}),
      ...(Array.isArray(defaultValues?.images) ? { images: defaultValues.images } : {}),
      ...(Array.isArray(defaultValues?.imageBlurs) ? { imageBlurs: defaultValues.imageBlurs } : {}),
    },
  })
  ...
  const updateParamRow = (id: string, patch: Partial<ParamRow>) => {
    setParamRows((prev) => prev.map((row) => (row.id === id ? { ...row, ...patch } : row)))
  }
    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          >
            <SelectTrigger className="w-full min-w-0 rounded-lg border-border bg-background text-sm text-foreground">
        ...
        {paramRows.length === 0 ? (
          ...
          <div className="space-y-2">
            {paramRows.map((row) => (
              <div
                key={row.id}
                className="grid grid-cols-[minmax(0,1fr)_auto] gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] [&>[role=combobox]]:col-span-2 sm:[&>[role=combobox]]:col-span-1"
              >
                <Select
                  value={row.key}
                  ...
                  <tbody className="divide-y divide-border">
                    {items.map((item) => (
                      <tr key={item.id} className="hover:bg-background transition-colors">
                        <td data-label="Работа" className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            {item.images[0] && (
                              <Image
                                ...
                                height={40}
                                sizes="40px"
                                className="w-10 h-10 shrink-0 rounded-md object-cover border border-border"
                              />
                            )}
                        ...
                        <td data-label="Категория" className="px-4 py-3 text-muted">
                        ...
                        <td data-label="Фото" className="px-4 py-3">
                          <span className="text-xs font-mono text-muted">{item.images.length}</span>
                        </td>

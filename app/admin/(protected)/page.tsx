...
export default async function AdminDashboard() {
  ...
  const STAT_CARDS = [
    { label: "Всего заказов", value: total, color: "text-[var(--foreground)]" },
    ...
    <div>
      ...
      <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)]">
          <h2 className="font-bold text-[var(--foreground)]" style={{ fontFamily: "Syne, sans-serif" }}>
            Последние заказы
          </h2>
        ...
        <div className="divide-y divide-[var(--border)]">
          {recent.length === 0 ? (
            <p className="p-5 text-sm text-[var(--muted)]">Заказов пока нет</p>
          ) : (
            recent.map((order) => (
              <Link
                key={order.id}
                href={`/admin/orders/${order.id}`}
                className="flex items-center gap-4 px-5 py-3.5 hover:bg-[var(--background)] transition-colors"
              >
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[var(--foreground)] truncate">{order.fullName}</p>
                  <p className="text-xs text-[var(--muted)] font-mono">{order.material} · {formatDate(order.createdAt)}</p>
                </div>

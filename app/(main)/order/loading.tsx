    ...
    <div className="min-h-screen bg-[var(--background)] py-8 pb-24 lg:pb-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <div className="h-10 w-48 rounded-lg bg-[var(--border)] animate-pulse" />
        </div>
          ...
          <div className="flex items-center justify-between">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="flex-1 flex items-center">
                ...
                {i < 5 && <div className="flex-1 h-px mx-3 bg-[var(--border)] animate-pulse" />}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="lg:grid lg:grid-cols-[1fr_320px] gap-8">
          ...
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 lg:p-8">
            <div className="h-8 w-56 rounded-lg bg-[var(--border)] animate-pulse mb-2" />
            <div className="h-4 w-80 rounded bg-[var(--border)] animate-pulse mb-8" />
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="h-32 rounded-xl border-2 border-[var(--border)] bg-[var(--border)] animate-pulse" />
              ))}
            </div>
          </div>
